...
# 1. Fix "Prise de Commande"
old_cmd = """elif navigation == "➕ Prise de Commande":
    st.title("➕ Nouvelle Commande")
    st.info("Le formulaire de commande classique est ici.")"""

new_cmd = """elif navigation == "➕ Prise de Commande":
    st.title("➕ Nouvelle Commande")
    ...
    if not df_clients.empty:
        ...
        if selected_client_id:
            ...
            with colB:
                ...
                date_livraison = st.date_input("Date souhaitée", dt.date.today() + dt.timedelta(days=1), format="DD/MM/YYYY")
                
                est_valide = True
                message_erreur = ""
                try:
                    # On essaie d'appeler la fonction existante
...
# 2. Fix "Planning"
old_plan = """elif navigation == "📅 Planning":
